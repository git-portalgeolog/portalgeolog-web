...
export default function PassengerConfirmButton({ token, alreadyAccepted }: PassengerConfirmButtonProps) {
  ...
  const handleConfirm = async () => {
    setStatus('loading');
    try {
      const res = await fetch(`/api/passenger-accept?token=${encodeURIComponent(token)}`);
      ...
      if (res.ok && data?.success) {
        ...
        setStatus('error');
        setMessage(data?.error || 'Não foi possível confirmar a viagem. Tente novamente mais tarde.');
      }
    } catch {
      setStatus('error');
  ...
  if (status === 'success') {
    return (
      <div className="space-y-4 text-center">
        ...
        <h2 className="text-xl font-black text-slate-900 uppercase tracking-wider">Viagem Confirmada</h2>
  ...
  if (status === 'already') {
    return (
      <div className="space-y-4 text-center">
        ...
        <h2 className="text-xl font-black text-slate-900 uppercase tracking-wider">Já Confirmada</h2>
        <p className="text-sm font-semibold text-slate-500">{message}</p>
      </div>
    );
  }
  ...
  if (status === 'error') {
    return (
      <div className="space-y-4 text-center">
        ...
        <h2 className="text-xl font-black text-slate-900 uppercase tracking-wider">Erro</h2>
        <p className="text-sm font-semibold text-slate-500">{message}</p>
        <button
  ...
  if (status === 'loading') {
    return (
      <div className="space-y-4 text-center">
        ...
        <h2 className="text-xl font-black text-slate-900 uppercase tracking-wider">Processando</h2>
        <p className="text-sm font-semibold text-slate-500">Registrando confirmação...</p>
      </div>
    );
  }
