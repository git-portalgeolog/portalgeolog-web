...
export default function Dashboard() {
  ...
  return (
    <div>
      <div className="bg-white dark:bg-[var(--color-geolog-secondary)] p-8 rounded-3xl shadow-2xl border border-gray-100 dark:border-white/5 overflow-hidden relative">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-500 dark:text-[var(--color-geolog-accent)] text-lg max-w-2xl leading-relaxed">
...
function StatCard({ title, value, color }: { title: string, value: string, color: string }) {
  return (
    <div className={`bg-gray-50 dark:bg-[var(--color-geolog-blue)] p-8 rounded-2xl border border-gray-100 dark:border-white/5 hover:shadow-xl transition-all border-l-4 ${color}`}>
      <p className="text-sm font-bold text-gray-400 dark:text-[var(--color-geolog-accent)] uppercase tracking-widest mb-2">{title}</p>
      <div className="flex items-end gap-2">
        <p className="text-5xl font-black dark:text-white">{value}</p>
        <span className="text-xs text-gray-400 mb-2 font-bold">UNIDADES</span>
      </div>
    </div>
  );
}
