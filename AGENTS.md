...
- **Proxy reverso:** Caddy na porta `80` → `localhost:21465`
- **Domínio Cloudflare:** `wppconnect.portalgeolog.com.br` (Proxied, SSL Full)
...
- Rate limit nas rotas `/api/whatsapp`, `/api/notify-passenger`, `/api/os-driver-accept`.
...
- O token JWT é gerado via: `POST /api/bot_cnh/{secretKey}/generate-token`
- O token retornado no campo `"full"` (ex: `bot_cnh:$2b$10$...`) é usado no header `Authorization: Bearer ...`.
...
- **Token válido atual:** `bot_cnh:$2b$10$ZonwxNXs_HYA5GeGHuitKOfnQQDvqNebBNTPMyI3zcQorxiSlR02a` (pode expirar se a sessão cair).
...
docker ps | grep wpp-server

# Ver logs do WPPConnect
docker logs wpp-server --tail 50

# Extrair link de pareamento (para reconectar WhatsApp)
docker logs wpp-server --tail 40 | grep -o 'code.*' | head -1

# Gerar novo token (usando secretKey atual do container)
curl -X POST 'http://localhost:21465/api/bot_cnh/THISISMYSECURETOKEN/generate-token'

# Testar envio local
curl -X POST 'http://localhost:21465/api/bot_cnh/send-message' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer bot_cnh:TOKEN_AQUI' \
  -d '{"phone":"5522992495653","message":"Teste","isGroup":false}'
```
...
4. Testar envio via proxy HTTPS (`https://wppconnect.portalgeolog.com.br`).
5. Confirmar que a sessão `bot_cnh` está conectada no WhatsApp.
