...
async function test() {
  ...
  console.log(
    "Buscando driver com phone ilike %" + last10 + "% ou %" + last11 + "%",
  );
  ...
  const driver = drivers?.find((d) => {
    ...
    return (
      dPhone === phoneDigits ||
      dPhone.endsWith(last10) ||
      dPhone.endsWith(last11)
    );
  });
  ...
  const { data: osList, error: osErr } = await supabase
    .from("ordens_servico")
    .select(
      "id, status_operacional, motorista, veiculo_id, driver_whatsapp_state, protocolo, os_number, trecho",
    )
